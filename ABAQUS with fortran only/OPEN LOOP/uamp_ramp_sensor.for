...
c     get sensor values first; note that 
      ...
      if (250 < -0.014*(svars(1)+svars(2)+svars(3)+svars(4))/18) then
          ampValueNew = 250
      else if (-0.014*(svars(1)+svars(2)+svars(3)+svars(4))/18<-250)then
          ampValueNew = -250
      else
          ampValueNew =-0.014*(svars(1)+svars(2)+svars(3)+svars(4))/18
      end if
